\chapter{Procesamiento Lenguaje Natural (\glsentryshort{pln})}

\section{¿Qué es el procesamiento de lenguaje natural?}

El \gls{pln}, es la rama de la \gls{ia} encargada de la interacción entre computadoras y el lenguaje humano. Este campo integra la lingüística computacional, basada en el modelado de reglas del lenguaje, con modelos estadísticos y algoritmos de aprendizaje profundo (\gls{dl}) y aprendizaje automático (\gls{ml}).
...
Las fases para realizar el \gls{pln} se pueden observar en la siguiente imagen:
...
\begin{enumerate}
  ...
  \item \textbf{Tokenización:} Es el proceso crítico de segmentar el flujo de texto en unidades atómicas significativas o \textit{tokens}.
        ...
        Dependiendo del modelo, los tokens pueden ser palabras completas, subpalabras (como en los modelos \gls{gpt}) o incluso caracteres. Una vez tokenizado el texto, cada token se asocia a un índice numérico único en un vocabulario, preparando el terreno para la generación de \textit{Embeddings}.

  \item \textbf{Etiquetado gramatical (\gls{pos} Tagging):} Asigna a cada token una categoría morfológica (sustantivo, verbo, adjetivo, etc.) basándose en su definición y contexto. Esto permite desambiguar el rol sintáctico de cada palabra.
...
\begin{enumerate}
  \item \textbf{\gls{ner} (NER):} Se ocupa de la identificación y clasificación de elementos clave en el texto, tales como nombres de personas, ubicaciones geográficas, organizaciones, etc.
        \\
  ...
  \item \textbf{\gls{wsd} (WSD):} Identifica el significado correcto de las palabras en función del contexto.
        \\
