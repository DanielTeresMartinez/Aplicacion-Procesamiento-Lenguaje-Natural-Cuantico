...
Para paliar el problema de la polisemia en modelos estáticos, propuestas como Sense2Vec (Trask et al., 2015) \cite{trask_sense2vec_2015} intentaron etiquetar las palabras con su categoría gramatical (separando, por ejemplo, el sustantivo del verbo). Por otro lado, para solventar el límite práctico de Word2Vec y GloVe en el manejo de palabras fuera del vocabulario y la morfología compleja, Bojanowski et al. presentaron en 2017 FastText \cite{bojanowski_enriching_2017}, que representaba cada palabra como una bolsa de n-gramas de caracteres (subpalabras), permitiendo al modelo componer vectores significativos para palabras nunca antes vistas. Sin embargo, todos estos modelos compartían un defecto, que eran estáticos, asignando un único vector fijo a cada palabra y colapsando todos sus significados posibles en uno solo, por lo que la solución definitiva requería un cambio de paradigma.
...
\subsection{Evolución histórica del PLNC}
...
La propuesta fundacional para resolverlo fue el modelo composicional-distribucional categórico, conocido como \gls{discocat} (\textit{Categorical Compositional Distributional}), formulado por Coecke, Sadrzadeh y Clark en 2008 \cite{clark_compositional_distributional_2008}. Este enfoque identificó una profunda correspondencia matemática, las categorías monoidales cerradas y compactas que rigen la Mecánica Cuántica Categórica (\gls{cqm}) para modelar la interacción de sistemas cuánticos comparten la misma estructura algebraica que las gramáticas de pregrupos utilizadas para describir la interacción sintáctica de las palabras. Aunque las primeras implementaciones de \gls{discocat} en hardware clásico demostraron una gran efectividad y superaron a otros modelos contemporáneos de la época \cite{clark_compositional_distributional_2008}, se toparon con un cuello de botella de escalabilidad. Dado que la composición del significado de frases se basa en el producto tensorial, la dimensionalidad del espacio vectorial resultante crece de forma exponencial con la longitud de la oración, haciendo que el cálculo clásico resulte intratable a gran escala.
...
De este modo, habiendo descrito las propiedades geométricas, conceptuales e históricas que fundamentan las representaciones vectoriales tanto en el dominio clásico como en el cuántico, el capítulo siguiente se dedica a detallar el funcionamiento de ambos algoritmos centrales. Asimismo, se presentarán una serie de propuestas de optimización introducidas en este trabajo sobre el modelo cuántico original para mejorar su convergencia y rendimiento.
