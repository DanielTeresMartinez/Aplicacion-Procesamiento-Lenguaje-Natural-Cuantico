...
Este capítulo aborda dicho problema en tres bloques progresivos. En primer lugar, se introduce el concepto de \textit{word embedding} y la característica que los definen, es decir, de qué modo la cercanía en el espacio vectorial codifica similitud semántica y por qué el espacio de Hilbert cuántico ofrece una alternativa estructuralmente afín al espacio euclídeo clásico. A continuación, se describe en detalle el algoritmo Word2Vec y en concreto su variante Skip-gram, modelo clásico de referencia de este trabajo. Finalmente, se expone QWord2Vec (la primera adaptación formal de Word2Vec al entorno cuántico definida en \cite{ohno_qword2vec_2025}), detallando el diseño de su circuito parametrizado, su función de pérdida y la estrategia de optimización \gls{qn-spsa}.
...
\section{Limitaciones identificadas y enfoque del trabajo}
\label{sec:limitaciones_enfoque}

A lo largo de este capítulo se ha recorrido la evolución de las representaciones vectoriales de palabras, partiendo de los fundamentos geométricos de los \textit{word embeddings} y del algoritmo clásico Word2Vec, hasta llegar a su adaptación al entorno cuántico mediante QWord2Vec. El modelo propuesto por Ohno \cite{ohno_qword2vec_2025} constituye, como se ha visto, la primera formulación de un circuito cuántico parametrizado concebido específicamente para aprender \textit{embeddings} de palabras, y representa, por tanto, el punto de partida sobre el que se asienta el presente trabajo.

No obstante, del análisis del estado del arte se desprenden varias limitaciones que condicionan el rendimiento del modelo cuántico. La más relevante se encuentra en el proceso de optimización. El entrenamiento original utiliza el optimizador \gls{spsa} y contiene un número muy elevado de iteraciones (hasta $20\,000$) para alcanzar la convergencia, lo que, unido al coste exponencial de la simulación clásica de circuitos, encarece de forma notable el entrenamiento. A esta dificultad se suma la fuerte dependencia del resultado respecto a la inicialización de los parámetros del circuito y a la propia naturaleza estocástica del optimizador, factores que dificultan una convergencia estable y reproducible.

Partiendo de esta base, el presente Trabajo de Fin de Grado se propone explorar vías de optimización matemática e inicialización de parámetros orientadas a mitigar estos problemas de convergencia. En concreto, se sustituye el optimizador original por \gls{qn-spsa}, que incorpora información sobre la geometría del espacio de estados cuánticos a través del tensor de Fubini-Study, y se introducen estrategias de ajuste fino de los hiperparámetros (\textit{Grid Search}) e inicialización informada de los parámetros (\textit{educated guess}) con el objetivo de acelerar y estabilizar el entrenamiento. El capítulo siguiente detalla el diseño experimental construido sobre estas ideas y evalúa empíricamente en qué medida dichas optimizaciones permiten a QWord2Vec aproximarse a la calidad de los \textit{embeddings} generados por Word2Vec así como si tiene algún efecto el utilizarlas.

Descritas en detalle las arquitecturas de ambos modelos, quedan por responder a la pregunta más importante para ver su viabilidad en la actualidad ¿son capaces los \textit{embeddings} cuánticos de preservar las relaciones semánticas con una calidad comparable a los clásicos? El capítulo siguiente recoge la experimentación llevada a cabo para responder a estas cuestiones, comparando Word2Vec y QWord2Vec de forma sistemática mediante métricas de calidad de los \textit{embeddings}, análisis de la función de pérdida y un estudio del impacto de ampliar el corpus de entrenamiento respecto al corpus utilizado en el artículo de referencia \cite{ohno_qword2vec_2025}.
