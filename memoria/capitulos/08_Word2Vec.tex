...
Para entender la mejora que supone \textit{Negative Sampling}, primero hay que entender la fórmula original matemática con la que se define el modelo básico de Skip-gram para luego así poder entender las mejoras que esta función de optimización ofrece \cite{tsang_skipgram_review_medium}.

El objetivo principal de Skip-gram es maximizar la media de probabilidad logarítmica. Es decir, viene definido por la siguiente fórmula:
...
\begin{itemize}
  \item $c$ es el tamaño del contexto de entrenamiento. Mayores valores de $c$ se traducen en más ejemplos de entrenamiento lo que, a su vez, ofrece una mayor precisión a costa de un mayor tiempo de entrenamiento.
  \item $T$ es el número total de palabras del corpus de entrenamiento.
  \item $w_t$ es una palabra de entrenamiento en la posición $t$.
\end{itemize}

La probabilidad $p(w_{t+j} \mid w_t)$ del modelo básico de Skip-gram se define mediante la función \textit{softmax} de la siguiente manera \cite{tsang_skipgram_review_medium}:
...
Esta formulación presenta un problema de escalabilidad: el coste de calcular $\nabla \log p(w_O \mid w_I)$ es proporcional a $W$, que habitualmente tiene un valor muy elevado (entre $10^5$ y $10^7$ términos), por lo que en el denominador del \textit{softmax} hay que sumar todos los valores exponenciales para todas las palabras del vocabulario. Cuando hay millones de palabras, hace que este proceso sea muy lento. Es precisamente este coste el que motiva la introducción del \textit{Negative Sampling} como función de optimización alternativa.
