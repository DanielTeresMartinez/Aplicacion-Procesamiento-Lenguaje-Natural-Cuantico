...
Para construir ambos conjuntos se ha generado primero un agrupamiento (\textit{ground truth}) de las palabras del vocabulario a partir del corpus teniendo en cuenta que los grupos fueran lo más equilibrados posible y sin que una misma palabra apareciera en distintos grupos. Estos fueron los cuatro clústeres semánticos generados:
...
En cuanto al flujo de entrenamiento, antes de comenzar la optimización se lleva a cabo una inicialización de los parámetros del circuito ya que se está entrenando desde cero. El procedimiento es el siguiente, se generan 10 puntos de inicio aleatorios y se evalúa la función de pérdida en cada uno de ellos, seleccionando aquel que presenta el valor más bajo como punto de partida para el optimizador. Esta estrategia reduce el riesgo de quedar atrapado en mínimos locales desde el inicio del entrenamiento obteniendo una ventaja en el inicio del entrenamiento.
...
La evaluación de los resultados obtenidos se divide en cuatro etapas: visualización de los \textit{embeddings} en el plano mediante un gráfico de dispersión 2D, análisis de las similitudes de palabras mediante matrices de similitud coseno, cuantificación de la similitud entre ambas representaciones mediante el coeficiente de correlación de Pearson, y reporte de las métricas finales específicas de cada modelo, \textit{cosine\_delta} para Word2Vec y \textit{top-2 accuracy} para QWord2Vec. Estas dos últimas métricas no son directamente comparables entre sí en términos numéricos, ya que cada una está diseñada para evaluar la propiedad semántica de su modelo correspondiente (uno en el espacio euclídeo y el otro en el espacio de Hilbert mediante probabilidades), pero ambas responden a la misma pregunta de si el modelo ha aprendido la estructura de similitud del corpus usado.
