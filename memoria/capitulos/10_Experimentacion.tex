...
El objetivo principal de esta experimentación es evaluar si los \textit{embeddings} generados por QWord2Vec son capaces de preservar relaciones semánticas entre palabras de forma comparable a los producidos por Word2Vec. Se explicará el corpus y la configuración de los hiperparámetros, para luego pasar al análisis de la función de pérdida durante el entrenamiento y terminar con la evaluación de la calidad de los \textit{embeddings} con distintas métricas usadas. Asimismo, se incluye un análisis comparativo de los tiempos de ejecución de ambos modelos, dado que el coste computacional es uno de los factores críticos en el contexto de la computación cuántica simulada.

La primera experimentación se realizará con el dataset usado en el artículo \cite{ohno_qword2vec_2025}. Luego habrá una segunda experimentación con un dataset más uniforme y con mayores instancias de entrenamiento partiendo del primer dataset utilizado para analizar así cómo mejora los modelos al aumentar un poco el número de instancias de entrenamiento.

\section{Metodología experimental}

A continuación se describe los pasos previos necesarios para llevar a cabo la experimentación. En primer lugar, se presenta el corpus empleado para el entrenamiento de ambos modelos, junto con el proceso de preprocesamiento aplicado en cada caso. al finalizar la explicación, se detallarán las herramientas y el entorno de ejecución utilizados, aspecto especialmente relevante en el caso de QWord2Vec, dado que la simulación clásica de circuitos cuánticos impone restricciones significativas sobre el tamaño del experimento realizable.
...
Los hiperparámetros de Word2Vec pueden agruparse en tres categorías ya que están los propios de la arquitectura Skip-Gram, los relacionados con Negative Sampling y el correspondiente al optimizador.
...
Respecto a los hiperparámetros de \textbf{Negative Sampling}, los parámetros explorados son \textit{negative} y \textit{ns\_exponent}. El primero indica el número de muestras negativas que se generan por cada par positivo durante el entrenamiento. El artículo original publicado por Google \cite{mikolov_skipgram_phrases_2013} recomendó valores entre 5 y 20 (siendo 20 para corpus de gran escala). Dado que el corpus empleado cuenta únicamente con 13 palabras únicas, se optó por explorar valores ligeramente inferiores al valor mínimo que recomentaba el artículod de Google. Estos valores fueron el $\{2, 3\}$ siendo seleccionado $\text{negative} = 2$.
...
La gráfica ilustra con claridad por qué el \textit{train loss} no es una métrica adecuada para seleccionar el mejor modelo. Desde las primeras iteraciones la pérdida alcanza valores de cero en varias ocasiones, mientras que en otras se dispara hasta picos puntuales superiores a $6$ o incluso $20$, oscilaciones inherentes al carácter estocástico de \textit{Negative Sampling} y al elevado valor de $\alpha = 0{,}59$ utilizado. Sin embargo, el \textit{cosine\_delta} sigue una trayectoria global ascendente que culmina con su mejor valor en torno a la época $400$, donde alcanza un \textit{cosine\_delta} de $0{,}8927$. Tras este punto, el modelo entra en una fase de degradación gradual de la métrica semántica que evidencia el comienzo del sobreajuste sobre un corpus tan reducido. El mecanismo de \textit{Best Model Checkpointing} aísla precisamente ese mejor punto de convergencia y descarta tanto las primeras épocas inestables como las posteriores degradadas.
...
La evaluación de los resultados obtenidos se divide en cuatro etapas: visualización de los \textit{embeddings} en el plano mediante un gráfico de dispersión 2D, análisis de las similitudes de palabras mediante matrices de similitud coseno, cuantificación de la similitud entre ambas representaciones mediante el coeficiente de correlación de Pearson, y reporte de las métricas finales específicas de cada modelo, \textit{cosine\_delta} para Word2Vec y \textit{top-2 accuracy} para QWord2Vec. Estas dos últimas métricas no son directamente comparables entre sí en términos numéricos, ya que cada una está diseñada para evaluar la propiedad semántica nativa de su modelo correspondiente, pero ambas responden a la misma pregunta cualitativa: si el modelo ha aprendido la estructura de similitud presente en el corpus.

\subsection{Visualización de los \textit{embeddings}}

La Figura \ref{fig:embeddings_comparison} muestra los \textit{embeddings} de ambos modelos proyectados en el plano. Para Word2Vec, cuyo tamaño de vector es de 2 dimensiones, no se requiere reducción dimensional. Para QWord2Vec, dado que los vectores de \textit{embedding} tienen dimensión $2^n = 2^4 = 16$ al emplear 4 qubits de sistema, se ha aplicado \gls{pca} para proyectarlos a 2 dimensiones, obteniendo una varianza explicada del $74{,}8\%$. Esta reducción implica una pérdida de información considerable respecto a la métrica real en el espacio completo, por lo que las distancias observadas en la proyección deben interpretarse con cautela.
...
La situación se invierte parcialmente en el clúster de sentimiento. \textit{Word2Vec} agrupa con alta similitud los pares \textit{i}--\textit{hate} y \textit{like}--\textit{hate} ($0{,}930$ y $0{,}850$ respectivamente), aunque el par \textit{i}--\textit{like} resulta sorprendentemente más bajo ($0{,}596$) por la dispersión de contextos de la palabra \textit{like} en el corpus. \textit{QWord2Vec} reproduce con claridad el par \textit{like}--\textit{hate}, pero \textit{i} queda algo más desconectada de las otras dos. Esto es consecuencia directa del esquema de etiquetas \textit{top}-$2$ del modelo cuántico: al construir la etiqueta de \textit{i} a partir únicamente de sus dos contextos más frecuentes, se descartan otras co-ocurrencias menores que sí pueden ser útiles para reflejar la cercanía semántica con \textit{like} y \textit{hate}, mientras que \textit{Word2Vec} sí las aprovecha.

Cabe señalar una limitación metodológica en la aplicación de la similitud coseno a \textit{QWord2Vec}. Las representaciones de este modelo son distribuciones de probabilidad sobre un espacio de $2^n$ estados, no vectores en un espacio euclídeo. La métrica teóricamente adecuada para comparar distribuciones de probabilidad es la \textbf{distancia de Hellinger} \cite{oecd_hellinger_distance}, que respeta la estructura geométrica del símplex de probabilidad. En la presente implementación se ha empleado la similitud coseno para ambos modelos por coherencia visual en la comparación, lo cual constituye una aproximación válida dado que el coseno sobre vectores no negativos mide proximidad angular entre perfiles de distribución. La sustitución de la similitud coseno por la distancia de Hellinger para la representación de \textit{QWord2Vec} se deja como posible mejora futura.

\subsection{Correlación de Pearson y métricas finales}

Para cuantificar en qué medida ambos modelos organizan el espacio de palabras de forma parecida, se ha calculado el coeficiente de correlación de Pearson entre los vectores de distancias coseno de \textit{Word2Vec} y \textit{QWord2Vec}, aplanando las matrices triangulares superiores de similitud en un único vector por modelo. El valor obtenido es de $0{,}2920$, lo que refleja una correlación baja entre las estructuras de similitud de ambos modelos. Esta métrica debe interpretarse como una \textit{aproximación}, ya que para \textit{QWord2Vec} se está utilizando la similitud coseno en lugar de su métrica nativa (la distancia de Hellinger sobre el símplex de probabilidad) y, además, los \textit{embeddings} cuánticos están definidos en un espacio de dimensión $2^4 = 16$ del que la proyección \gls{pca} solo conserva un $74{,}8\%$ de la varianza. Aun así, sirve para evaluar de forma global cómo de parecida es la disposición de las palabras en ambos espacios y constituye la principal magnitud comparativa que reporta también el trabajo de referencia \cite{ohno_qword2vec_2025}, en el que se obtiene un valor de $0{,}81$ sobre un experimento equivalente.

Complementariamente, se reportan las métricas finales específicas de cada modelo. \textit{Word2Vec} alcanza un \textit{cosine\_delta} de $0{,}8927$, valor cercano al máximo teórico observado sobre este corpus, que indica una clara separación entre clústeres semánticos en el espacio euclídeo. \textit{QWord2Vec}, por su parte, alcanza una \textit{top-2 accuracy} de $0{,}8462$, lo que significa que el circuito coloca correctamente la mayor masa de probabilidad en los dos contextos esperados para $9$ de las $13$ palabras del vocabulario. Como se ha indicado anteriormente, ambas métricas no son numéricamente comparables entre sí al estar diseñadas sobre espacios y supuestos distintos, pero las dos confirman que cada modelo ha aprendido una representación semánticamente coherente sobre el corpus utilizado.
...
En segundo lugar, las métricas finales específicas de cada modelo revelan que ambos han aprendido representaciones de calidad comparable sobre el corpus, aunque cada una se mida de forma propia. \textit{Word2Vec} alcanza un \textit{cosine\_delta} de $0{,}8927$, indicando una clara separación entre los clústeres semánticos en el espacio euclídeo, y \textit{QWord2Vec} alcanza una \textit{top-2 accuracy} de $0{,}8462$, predicción correcta de los dos contextos esperados para $9$ de las $13$ palabras del vocabulario. Este resultado es especialmente destacable por dos razones. Por un lado, \textit{QWord2Vec} ha sido entrenado con menos información, ya que utiliza únicamente los dos contextos más frecuentes por palabra en lugar de todos los pares (palabra, contexto) que aprovecha \textit{Word2Vec}. Por otro lado, ninguno de los dos modelos optimiza directamente la métrica final que se reporta: el \textit{cosine\_delta} guía la selección del mejor \textit{checkpoint} en \textit{Word2Vec} sin formar parte de su función de pérdida, y la \textit{top-2 accuracy} se calcula a partir de la salida del circuito sin que aparezca explícitamente en la función de pérdida que minimiza \gls{qn-spsa}. Que ambos modelos alcancen valores elevados en sus respectivas métricas constituye, por tanto, una evidencia indirecta pero consistente de que han aprendido la estructura semántica subyacente al corpus.
...
Una vez analizados los resultados obtenidos sobre el corpus inicial, se ha llevado a cabo una segunda fase de experimentación con el objetivo de evaluar en qué medida las limitaciones observadas son atribuibles al tamaño y al desequilibrio del corpus original. Esta segunda fase replica el procedimiento descrito en las secciones anteriores sobre un corpus ampliado, reajustando los hiperparámetros que requieren un nuevo equilibrio dada la modificación de la señal de entrenamiento. Para evitar redundancias, esta sección describe únicamente los aspectos que cambian respecto a la fase inicial, asumiendo el resto de la metodología, las métricas de evaluación y los criterios de parada ya definidos.

\subsection{Ampliación del corpus}

El nuevo corpus extiende el original hasta alcanzar las \textbf{64 frases}, manteniendo el mismo vocabulario de 13 palabras (\textit{dog}, \textit{cat}, \textit{animal}, \textit{eyes}, \textit{apple}, \textit{fish}, \textit{milk}, \textit{book}, \textit{music}, \textit{movie}, \textit{i}, \textit{like}, \textit{hate}). Las frases se han redistribuido de modo que cada uno de los cuatro clústeres semánticos definidos en la sección de entrenamiento (\textit{animal}, \textit{food}, \textit{culture} y \textit{sentiment}) reciba aproximadamente 16 frases, dando lugar a un corpus equilibrado en el que ningún clúster domina sobre los demás.

Este equilibrio resulta beneficioso para ambos modelos. En \textit{Word2Vec} evita que los pares de co-ocurrencia procedentes del clúster más numeroso dominen el gradiente, lo que en la fase inicial se tradujo en distorsiones como la cercanía artificial entre \textit{apple} y \textit{hate} o la similitud espuria entre \textit{book} y \textit{movie}. En \textit{QWord2Vec}, donde los vectores de etiqueta se construyen a partir de los dos contextos más frecuentes de cada palabra, una distribución de frases más uniforme genera etiquetas más representativas del vocabulario completo y reduce los sesgos hacia palabras con frecuencia desproporcionada.

\subsection{Reajuste de hiperparámetros}

El cambio en la cantidad y diversidad de la señal de entrenamiento exige reevaluar los hiperparámetros de ambos modelos. En consecuencia, se ha repetido el procedimiento de \textit{Grid Search} descrito previamente, manteniendo los mismos rangos generales y aplicando ajustes locales en torno a los valores que resultaron óptimos en la fase inicial.
...
El cambio más significativo es la reducción drástica de la tasa de aprendizaje inicial. En la fase anterior, el reducido tamaño del corpus exigía pasos de gradiente agresivos para escapar de mínimos locales y aprovechar al máximo cada época. Con 64 frases, el modelo recibe una señal mucho más rica por época y converge con un \textit{alpha} considerablemente más suave, lo que se traduce en una optimización más estable y menos errática. El aumento de \textit{vector\_size} de $2$ a $4$ permite obtener representaciones más expresivas: con 13 frases una dimensionalidad mayor habría provocado sobreajuste, mientras que con 64 frases existe señal suficiente para poblar las cuatro dimensiones de forma significativa. Para preservar la posibilidad de visualizar los \textit{embeddings} en el plano se aplica posteriormente \gls{pca} sobre las cuatro dimensiones, conservando una varianza explicada en torno al $72\%$. Por último, el cambio en \textit{ns\_exponent} de $0{,}0$ a $0{,}4$ refleja que, al equilibrarse el corpus, ya no resulta necesario forzar una distribución uniforme de muestras negativas para compensar el desequilibrio léxico, siendo más adecuado un valor intermedio que conserve cierta proporcionalidad con la frecuencia real de cada palabra.

En el caso de \textbf{QWord2Vec}, el reajuste se centra exclusivamente en el parámetro $C$ de la función de pérdida, que pondera la contribución del coeficiente de correlación de Pearson frente a la entropía cruzada. La búsqueda exploró los valores $C \in \{1, 2, 3, 4\}$, seleccionándose finalmente $C = 3$ frente al $C = 2$ utilizado en la fase inicial. Un valor mayor de $C$ penaliza con más fuerza la falta de correlación entre las distancias del espacio de Hilbert y las del espacio euclídeo de \textit{Word2Vec}, empujando al circuito a alinear su estructura geométrica con la semántica del corpus. Con un corpus más amplio y rico en co-ocurrencias, la señal de correlación es más fiable y soporta esta penalización adicional sin desestabilizar el entrenamiento. El resto de hiperparámetros del optimizador \gls{qn-spsa} se mantiene sin cambios respecto a la fase inicial, ya que el \textit{Grid Search} no encontró configuraciones mejores variando el resto de parámetros.

\subsection{Resultados de Word2Vec}

La Figura \ref{fig:loss_curves_word2vec_v2} muestra la evolución del entrenamiento de \textit{Word2Vec} sobre el corpus ampliado. La curva de \textit{cosine\_delta} presenta una tendencia ascendente claramente más estable que en la fase inicial, alcanzando el mejor punto en torno a la época 350 con un valor de $0{,}9360$. La pérdida de \textit{Negative Sampling} mantiene cierto grado de oscilación, comportamiento esperable dada su naturaleza estocástica, pero se sitúa en un rango más controlado al haberse reducido la tasa de aprendizaje.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\linewidth]{imagenes/lossCurvesWord2Vec_v2.png}
...
El análisis de las similitudes coseno por pares dentro de cada clúster, mostrado en la Figura \ref{fig:cosine_similarity_v2}, revela una mejora notable en la cohesión interna de la mayoría de los grupos. En el clúster \textit{animal}, los pares \textit{dog}--\textit{cat} y \textit{dog}--\textit{animal} aumentan su similitud desde $0{,}326$ y $0{,}572$ hasta $0{,}789$ y $0{,}916$ respectivamente. En el clúster \textit{food}, el par \textit{fish}--\textit{milk} pasa de $0{,}702$ a $0{,}953$, mientras que \textit{apple}--\textit{fish} se mantiene en torno a $0{,}914$. En el clúster \textit{culture}, el par \textit{music}--\textit{movie} mejora drásticamente de $0{,}276$ a $0{,}934$, recuperando la cohesión que en la fase inicial se concentraba artificialmente en \textit{book}--\textit{movie}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.95\linewidth]{imagenes/cosine_similarity_comparison_v2.png}
...
La excepción más llamativa es precisamente el par \textit{book}--\textit{movie}, que retrocede de $0{,}998$ a $0{,}167$. Lejos de constituir un empeoramiento, este descenso refleja la corrección del desequilibrio del corpus original: en la fase inicial, la palabra \textit{book} compartía contexto casi exclusivamente con \textit{movie} a través de la única frase que las relacionaba, generando una similitud artificialmente elevada. Al disponer ahora de contextos variados para \textit{book}, esta correlación espuria desaparece y el clúster \textit{culture} se sostiene sobre la cohesión real entre \textit{music} y \textit{movie}, dejando a \textit{book} en una posición algo más periférica pero coherente con sus co-ocurrencias en el nuevo corpus.

\subsection{Resultados de QWord2Vec}

El entrenamiento de \textit{QWord2Vec} sobre el corpus ampliado y con la nueva configuración alcanza un \textit{error rate} de $0{,}1154$ en la mejor iteración, equivalente a una \textit{top-2 accuracy} del $84{,}62\%$. El mejor punto se localiza en torno a la iteración 400 sobre un total de 800, a partir de la cual la optimización oscila sin mejora adicional, comportamiento habitual de \gls{qn-spsa} en espacios de alta dimensión con pocos datos. La correlación de Pearson final entre las distancias coseno de \textit{Word2Vec} y \textit{QWord2Vec} se sitúa en $0{,}4940$, valor inferior al $0{,}81$ reportado por el trabajo de referencia \cite{ohno_qword2vec_2025} pero apreciablemente superior al obtenido en la fase inicial. La Figura \ref{fig:loss_curves_qword2vec_v2} muestra la evolución conjunta de la función de pérdida y del \textit{error rate} durante el entrenamiento.
...
La Tabla \ref{tab:qword2vec_v2_diagnostico} presenta el diagnóstico por palabra, comparando los dos contextos más frecuentes esperados (etiqueta de entrenamiento) con los dos picos predichos por el circuito tras la inferencia. El modelo acierta los dos contextos esperados en 9 de las 13 palabras del vocabulario.

\begin{table}[h]
  \centering
  \begin{tabular}{|l|l|l|c|}
    \hline
    \textbf{Palabra} & \textbf{Predicción top-2} & \textbf{Etiqueta esperada} & \textbf{Acierto} \\
    \hline
    animal           & dog, cat                  & cat, dog                   & \checkmark       \\
    apple            & milk, fish                & milk, fish                 & \checkmark       \\
    book             & movie, music              & music, movie               & \checkmark       \\
    cat              & dog, i                    & animal, dog                & $\times$         \\
    dog              & cat, dog                  & cat, eyes                  & $\times$         \\
    eyes             & dog, cat                  & cat, dog                   & \checkmark       \\
    fish             & milk, apple               & milk, apple                & \checkmark       \\
    hate             & dog, i                    & i, dog                     & \checkmark       \\
    i                & like, hate                & like, hate                 & \checkmark       \\
    like             & dog, i                    & i, dog                     & \checkmark       \\
    milk             & fish, milk                & apple, fish                & $\times$         \\
    movie            & book, music               & music, book                & \checkmark       \\
    music            & movie, fish               & movie, book                & $\times$         \\
    \hline
  \end{tabular}
  \caption{Diagnóstico por palabra del modelo \textit{QWord2Vec} con el corpus ampliado: predicción \textit{top-2} frente a etiqueta esperada.}
  \label{tab:qword2vec_v2_diagnostico}
\end{table}

Los cuatro fallos se concentran en \textit{cat}, \textit{dog}, \textit{milk} y \textit{music}, palabras cuyas etiquetas esperadas (\textit{dog} e \textit{i}, principalmente) presentan en el corpus una alta conectividad con muchos otros términos. Esta dispersión de contextos dificulta que el circuito separe con claridad la señal correspondiente a cada par, especialmente con un número reducido de qubits.

La Figura \ref{fig:embeddings_comparison_v2} muestra la proyección \gls{pca} de las distribuciones de probabilidad de los \textit{embeddings} cuánticos junto a los de \textit{Word2Vec}, con una varianza explicada del $72{,}5\%$, similar a la obtenida en la fase inicial. La separación entre clústeres es claramente reconocible: los términos del grupo \textit{animal} se concentran en valores positivos de PC1; los del grupo \textit{food} en PC1 negativo y PC2 positivo; los de \textit{culture} en PC1 negativo y PC2 negativo; y los de \textit{sentiment} ocupan una región intermedia.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.95\linewidth]{imagenes/embeddings_comparison_v2.png}
...
La mejora es generalizada en la mayoría de los pares intra-clúster, con un \textit{cosine\_delta} global que aumenta en aproximadamente $5$ puntos porcentuales. Las únicas métricas que retroceden corresponden a casos en los que la fase inicial había generado relaciones artificialmente elevadas debido al desequilibrio del corpus, cuya corrección era de hecho un objetivo implícito de la ampliación.

En el caso de \textit{QWord2Vec}, la comparación numérica directa entre fases es limitada, ya que los pesos de la fase inicial no se reevaluaron con la versión final del código. No obstante, pueden destacarse tres observaciones. En primer lugar, el \textit{error rate} de entrenamiento se mantiene en $0{,}1154$ en ambas fases, lo que sugiere que el circuito alcanza una fracción similar de aciertos con independencia del tamaño del corpus. En segundo lugar, la separación visual entre clústeres en la proyección \gls{pca} es notablemente más nítida en la segunda fase, especialmente para los grupos \textit{food} y \textit{culture}, que en la fase inicial aparecían más solapados. En tercer lugar, la correlación de Pearson aumenta sensiblemente, lo que indica una mejor alineación entre la geometría aprendida por el circuito cuántico y la del modelo clásico.

La interpretación conjunta de estos resultados es que, al ampliar el corpus, el modelo cuántico aprende representaciones cualitativamente más descriptivas, aunque la métrica de entrenamiento permanezca aproximadamente constante. El aumento del corpus introduce simultáneamente más señal útil y más co-ocurrencias distintas que el circuito debe discriminar, manteniendo el nivel de error pese a una mejor estructuración semántica del espacio de \textit{embeddings}. En conjunto, la segunda fase confirma que ambos modelos escalan favorablemente con un corpus equilibrado, siendo \textit{Word2Vec} el que más se beneficia en términos numéricos y \textit{QWord2Vec} el que más mejora cualitativamente en la organización del espacio de Hilbert.
