...
De aquí se pueden sacar bastantes conclusiones acerca de este algoritmo. La primera de ellas es relacionada a la pérdida. Tal y como se indicó en la definición de esta función de pérdida, viene controlada por un parámetro $C$ que sirve para balancear. Como este parámetro multiplica a la entropía cruzada que se calcula entre los valores de embeddings de qword2vec y los training labels utilizados (recordar que tienen un valor de 0,5 dada la palabra de contexto, es decir las 2 palabras más cercanas a la palabra de referncia tiene valor de 0,5), Entonces al tener un loss negativo, significa que el modelo está aprendiendo una buena geometría de distancia entre las palabras de Qword2vec y las de entrenamiento. Por ello el modelo puede minimizar esta función de pérdida por debajo del 0 y esto es una buena señal ya que significa que el optimizador está haciendo su trabajo.

En cuanto al error rate, se puede apreciar cómo va disminuyendo a la par que la función de pérdida, esto significa que las 2 métricas son métricas muy buenas para calcular si el modelo está aprendiendo bien o no está aprendiendo. El error rate conseguido es muy bajo del 0,1, es decir, si le proporcionas una palabra de entrada de este corpus, con solo un 0,1 de probabilidad de fallo te va a decir las 2 palabras inmediatamente adyacentes que tiene (tamaño de ventana = 1) por lo que quiere decir que el modelo ha aprendido muy bien.

A partir de la época 360, se puede apreciar como la tendencia de la pérdida y el ratio de error pasa de ser convexo a ser cóncavo. Eso significa que el modelo ha llegado al máximo de lo que podía generalizar con el corpus que se tiene y ahora está sobreentrenando. Luego se puede apreciar como 100 iteraciones después salta el Early Stopping evitando así ejecutar más épocas de las necesarias.

Otra cosa importante que se ha visto es que en comparación al máximo de 20.000 épocas que le puso el autor original, con este algoritmo ha tardado 360 épocas en converger hasta el mejor mínimo encontrado. Esto indica como los hiperparámetros configurados para esta función de optimización han sido bastante buenos y se corresponde con la teoría de que \gls{qn-spsa} está diseñado para que converga en pocas iteraciones.

Además otro resultado curioso, es que tanto con el Grid Search como con la ejecución final con más iteraciones y shots, aún así ha convergido antes de que se aplique el tensor métrico de Fubini-Study, con esto quiero demostrar como para un problema de pequeñas dimensiones a pesar de que sobre el papel este algoritmo \gls{qn-spsa} ofrecía mejores resultados que el \gls{qng} o el \gls{spsa} por separado, al final para este dataset tan pequeño y con tan pocos qubits, ha resultado ser que la mejor opción encontrada en realidad era ejecutar \gls{spsa} finetuneando los valores de learning rate inicial, perturbación inicial y utilizando el parámetro de blocking. Lo más probable es que este algoritmo para un corpus más complejo y con mayores qubits a utilizar, se comportara mejor ya que tendría más parametros que calcular.
