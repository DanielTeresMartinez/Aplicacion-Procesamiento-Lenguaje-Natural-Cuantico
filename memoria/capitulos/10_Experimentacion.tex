...
Complementariamente, se reportan las métricas finales específicas de cada modelo. \textit{Word2Vec} alcanza un \textit{cosine\_delta} de $0{,}8927$, que indica una clara separación entre clústeres semánticos en el espacio euclídeo. \textit{QWord2Vec}, por su parte, alcanza una \textit{top-2 accuracy} de $0{,}8462$, lo que significa que el circuito predice la probabilidad en los dos contextos esperados para $9$ de las $13$ palabras del vocabulario. Como se ha indicado anteriormente, ambas métricas no son numéricamente comparables entre sí al estar diseñadas sobre espacios y características distintas, pero las dos confirman que cada modelo ha aprendido una representación semánticamente coherente sobre el corpus utilizado.
...
En segundo lugar, las métricas finales específicas de cada modelo revelan que ambos han aprendido representaciones de calidad comparable sobre el corpus, aunque cada una se mida de acuerdo a sus características. \textit{Word2Vec} alcanza un \textit{cosine\_delta} de $0{,}8927$, indicando una clara separación entre los clústeres definidos en el espacio euclídeo, y \textit{QWord2Vec} alcanza una \textit{top-2 accuracy} de $0{,}8462$, predicción correcta de los dos contextos esperados para $9$ de las $13$ palabras del vocabulario. Este resultado es especialmente destacable por dos razones. Por un lado, \textit{QWord2Vec} ha sido entrenado con menos información, ya que utiliza únicamente los dos contextos más frecuentes por palabra en lugar de todos los pares (palabra, contexto) el cual aprovecha \textit{Word2Vec}. Por último, ninguno de los dos modelos optimiza directamente la métrica final utilizada. El \textit{cosine\_delta} guía la selección del mejor \textit{checkpoint} en \textit{Word2Vec} sin formar parte de su función de pérdida, y la \textit{top-2 accuracy} se calcula a partir de la salida del circuito sin que intervenga en la función de pérdida que minimiza \gls{qn-spsa}. Que ambos modelos alcancen valores elevados en sus respectivas métricas constituye, por tanto, una evidencia indirecta pero consistente de que han aprendido la estructura semántica subyacente al corpus.
