...
# from sklearn.cluster import KMeans          # usado por evaluate (comentado)
# from sklearn.metrics import confusion_matrix # usado por evaluate (comentado)
# from scipy.optimize import linear_sum_assignment  # usado por evaluate (comentado)
import numpy as np
...
def most_similar_error_rate(model, sentences, window, debug=False):
    """
    Tasa de error análoga a la de Q-Word2Vec basada en vecinos más cercanos.
    Para cada palabra w, los 2 vecinos más similares por coseno deben coincidir
    con los 2 co-ocurrentes principales en el corpus usando la misma ventana
...
def evaluate_error_rate(model, sentences):
    """Tasa de error basada en most_similar (análoga a Q-Word2Vec).
    Minimizar — guía el early stopping igual que en Q-Word2Vec.
    """
    return most_similar_error_rate(model, sentences, window=BEST_PARAMS["window"])
...
class LossCallback(CallbackAny2Vec):
    ...
    def on_epoch_end(self, model):
        ...
        if self._epoch % PRINT_EVERY == 0:
            val_score = evaluate_error_rate(model, self.sentences)
            ...
            print(
                ...
                f" | error_rate = {val_score:.4f}{improved_tag}"
    ...
    def on_train_end(self, model):
        if self.val_scores:
            ...
            print(
                f"Error rate (último checkpoint, época {last_epoch}): {last_score:.4f}"
            )
...
if __name__ == "__main__":
    ...
    loss_cb = LossCallback(sentences)
    ...
    # ── 4. Evaluación final ───────────────────────────────────────────────────
    ms_error_rate = evaluate_error_rate(model, sentences)
    print(f"\nMost-similar error rate final:   {ms_error_rate:.4f}")
